...
var patern = 0;
    ...
    if(patern > 0){
            ...
            tamakawa.centerY = tamaY;
            // radian =Math.atan2(playerEntityGroup.centerY-tamaY,playerEntityGroup.centerX-tamaX);
            angle = angle2+ 25*(-1)**plus;
            angle2 = angle;
            tamakawa.moveX = 6 * Math.cos(angle* (Math.PI / 180));
            tamakawa.moveY = 6 * Math.sin(angle * (Math.PI / 180));
            for (i = a; i <= b; i++) {
                Random = random(-30, 30);
                ds = random(1, 4) ;
                ds = ds.toFixed();
        ...
        if (core.input.left && player2.x > 0) {
            if (core.input.x == true) {
                if (core.input.up || core.input.down) {
                    playerEntityGroup.x = playerEntityGroup.x - 1.2 / Math.SQRT2;
                } else {
